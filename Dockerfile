...
# SRT
RUN apk add luajit luajit-dev libsrt libsrt-dev

# Build
RUN git clone https://github.com/maltris/MonaServer2.git
WORKDIR /usr/src/MonaServer2
RUN make ENABLE_SRT=1

RUN cp MonaTiny/MonaTiny MonaServer/MonaServer MonaTiny/cert.pem MonaTiny/key.pem /usr/local/bin/
...
RUN apk add --no-cache libgcc libstdc++ luajit libsrt
